...
vector<Center*> orderParallelogramCorners(const vector<Center*>& pts) {
    if (pts.size() != 4) throw invalid_argument("Need exactly 4 points");

    int iTL = 0, iTR = 0, iBL = 0, iBR = 0;
    double minSum = numeric_limits<double>::infinity();
    ...
    for (int i = 0; i < 4; ++i) {
        double x = static_cast<double>(pts[i]->cx);
        double y = static_cast<double>(pts[i]->cy);
        double s = x + y;    // x + y
        double d = y - x;    // y - x

        if (s < minSum) { minSum = s; iTL = i; }
        if (s > maxSum) { maxSum = s; iBR = i; }
        if (d < minDiff){ minDiff = d; iTR = i; }
        if (d > maxDiff){ maxDiff = d; iBL = i; }
    }

    return { pts[iTL], pts[iTR], pts[iBL], pts[iBR] }; // [TL, TR, BL, BR]
}
                               ...
                               const vector<Center*>& corners)
{
    ...
    // corners[3] = p22 (not needed for basis)
